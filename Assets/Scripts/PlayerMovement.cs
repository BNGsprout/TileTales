...
{
    [SerializeField] float moveSpeed = 7.3f;
    [SerializeField] float jumpSpeed = 12.16f;
    ...
    void OnSuperJump(InputValue value)
    {
        
        if(!myFeetCollider.IsTouchingLayers(LayerMask.GetMask("Ground")) || hasSuperJumped == true)
        {
            return;
        }
        ...
        if(value.isPressed)
        {
            ...
            {
                myRigidbody.linearVelocity += new Vector2 (0f, superJumpSpeed);
            }
            else
            {
                hasSuperJumped = true;
                myRigidbody.linearVelocity += new Vector2 (0f, superJumpSpeed);
                superJumpSpeed = 0f;
                StartCoroutine(SuperJumpGo());
            }
        }
        
    }

    IEnumerator SuperJumpGo()
    {
